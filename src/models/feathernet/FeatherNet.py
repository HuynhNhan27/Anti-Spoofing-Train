...
#         building last several layers        
        ...
        # Dynamic FC input features calculation based on input_size
        with torch.no_grad():
            dummy = torch.zeros(1, 3, input_size, input_size)
            x_feat = self.features(dummy)
            x_feat = self.final_DW(x_feat)
            num_features = x_feat.view(1, -1).size(1)

        self.fc = nn.Linear(num_features, n_class)
...
def FeatherNetA(n_class=2, input_size=224):
    model = FeatherNet(n_class=n_class, input_size=input_size, se = True)
    return model

def FeatherNetB(n_class=2, input_size=224):
    model = FeatherNet(n_class=n_class, input_size=input_size, se = True, avgdown=True)
    return model
